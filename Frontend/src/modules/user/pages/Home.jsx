...
export function Home() {
  ...
  const offerSets = useMemo(
    () =>
      homePage?.specialOffers?.offerSets || [
        {
          images: [
            "/assets/advertisment/1.png",
            "/assets/advertisment/3.png",
            "/assets/advertisment/2.png",
          ],
        },
      ],
    [homePage?.specialOffers?.offerSets],
  ...
  const currentOfferSet =
    offerSets[adIndex] && offerSets[adIndex].images
      ? offerSets[adIndex].images
      : [];

  // Hero section images logic
  const heroImages = useMemo(() => {
    const images = homePage?.hero?.images;
    const singleImage = homePage?.hero?.image;

    if (images && images.length > 1) {
      return images;
    }
    return [singleImage || (images && images[0])];
  }, [homePage?.hero?.images, homePage?.hero?.image]);
    ...
    <motion.div
            ...
            <motion.div
              ...
              <motion.h1
                ...
                {homePage.hero.heading.split(" ").map((word, i) =>
                  word === "POWER" ? (
                    <span key={i} className="text-primary not-italic text-glow">
                      POWER{" "}
                    </span>
                  ) : (
                    word + " "
      ...
      <section className="w-full mt-12 mb-12 bg-primary py-12">
        ...
        <div className="flex gap-6 md:gap-12 overflow-x-auto py-8 px-8 snap-x hide-scrollbar justify-start w-full border-b border-border/5 bg-transparent backdrop-blur-sm">
          ...
          <a
            href="/products"
            className="flex flex-col items-center gap-2 min-w-[64px] md:min-w-[80px] group cursor-pointer snap-start px-2">
            ...
            <span className="text-[8px] md:text-xs font-bold uppercase tracking-wider text-primary-foreground/90 group-hover:text-white transition-colors text-center mt-1">
              All
            </span>
          </a>
          ...
          {backendCategories.map((cat, i) => (
            <a
              key={cat._id || i}
              href={`/products?category=${cat.categoryName.toLowerCase()}`}
              className="flex flex-col items-center gap-2 min-w-[64px] md:min-w-[80px] group cursor-pointer snap-start px-2">
              <div className="w-16 h-16 md:w-24 md:h-24 rounded-full p-1 bg-white/20 dark:bg-white/5 group-hover:bg-white/40 transition-all duration-300 transform group-hover:-translate-y-3 shadow-md group-hover:shadow-xl">
                <div className="w-full h-full rounded-full overflow-hidden border-2 border-white/80 dark:border-white/20 shadow-sm group-hover:scale-105 transition-transform duration-300 relative">
                  <img
                    src={
                      cat.image ||
                      "/assets/products/WhatsApp Image 2026-01-10 at 16.10.55.jpeg"
                    }
        ...
        <motion.div
          ...
          <div className="flex items-center gap-2 md:gap-8 shrink-0">
            ...
            <CTALink to={homePage.featuredSection.ctaLink}>
              <Button
                variant="link"
                className="font-black text-xs md:text-xl tracking-tighter uppercase italic group p-0 h-auto gap-1 md:gap-3 shrink-0">
          ...
          {homePage.categories.slice(0, 1).map((cat) => (
            <motion.div
              ...
              <div className="relative z-10">
                ...
                <p className="max-w-xs text-muted-foreground font-black mb-3 md:mb-10 uppercase tracking-[0.2em] text-[8px] md:text-xs opacity-80">
                  {cat.description}
                </p>
                <CTALink to={cat.ctaLink} className="w-fit">
                  <Button
                    premium
                    size="sm"
                    className="rounded-full px-6 md:px-12 h-8 md:h-14 font-black italic shadow-xl text-xs md:text-base">
                    {cat.ctaText}
                  </Button>
                </CTALink>
              </div>
          ...
          <div className="grid grid-cols-1 gap-8">
            {homePage.categories.slice(1).map((cat, idx) => (
              <motion.div
                ...
                />
                <div className="relative z-10">
                  <h3 className="text-3xl md:text-5xl font-black italic tracking-tighter uppercase mb-2 leading-none">
                    {cat.title}
                  </h3>
                  <p className="text-[10px] md:text-xs text-muted-foreground font-black uppercase tracking-[0.2em] mb-4 md:mb-8 opacity-70">
                    {cat.description}
                  </p>
                  <CTALink to={cat.ctaLink} className="w-fit">
                    <Button
                      variant="outline"
                      className="rounded-full border-2 border-foreground/10 font-black italic h-10 md:h-12 px-6 md:px-8 hover:bg-foreground hover:text-background transition-colors shadow-lg text-xs md:text-sm">
                      {cat.ctaText}
                    </Button>
                  </CTALink>
                </div>
          ...
          {homePage.trustMarkers.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                <div
                  className={`w-[80px] h-[80px] md:w-[280px] md:h-[280px] ${bgColor} rounded-full flex flex-col items-center justify-center text-center p-1 md:px-8 md:py-6 text-white transition-transform duration-300 hover:scale-105 z-10 relative shadow-md`}>
                  ...
                  <h3 className="text-[8px] md:text-lg font-bold uppercase tracking-wider mb-0 md:mb-1 font-['Oswald'] leading-tight max-w-[90%] truncate md:overflow-visible md:whitespace-normal">
                    {item.title}
                  </h3>
                ...
                <div
                  className={`absolute -bottom-1 md:-bottom-4 left-1/2 -translate-x-1/2 w-3/4 h-2 md:h-8 ${bgColor} opacity-20 blur-md md:blur-xl rounded-[100%]`}
                />
              </motion.div>
            );
          })}
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-3 gap-2 md:grid-cols-3 md:gap-8 items-center justify-items-center relative z-20 px-0 md:px-0">
            ...
            <div className="w-full flex justify-center">
              <img
                src={currentOfferSet[0] || "/placeholder.jpg"}
                alt="Ad 1"
                className="w-full h-auto drop-shadow-lg rounded-lg md:rounded-xl"
              />
            </div>

            <div className="w-full flex justify-center md:scale-125 z-10">
              <img
                src={currentOfferSet[1] || "/placeholder.jpg"}
                alt="Ad Main"
                className="w-full h-auto drop-shadow-xl rounded-lg md:rounded-xl"
              />
            </div>

            <div className="w-full flex justify-center">
              <img
                src={currentOfferSet[2] || "/placeholder.jpg"}
                alt="Ad 2"
                className="w-full h-auto drop-shadow-lg rounded-lg md:rounded-xl"
              />
            </div>
      ...
      <div className="w-full bg-background/80 backdrop-blur-md border-t border-border/10 py-6 flex flex-col md:flex-row items-center justify-center gap-3 relative z-20">
        <span className="text-muted-foreground text-[10px] md:text-xs uppercase tracking-[0.3em] font-bold">
          Have questions?
        </span>
      ...
      <div className="w-full overflow-hidden bg-background py-2 relative -mb-4 z-10">
        {" "}
        {/* Negative margin to blend with footer if needed */}
        <motion.div
          initial={{ x: "-30vw" }}
          animate={{ x: "100vw" }}
          transition={{
