...
export default function AdminVerifyResetOtp() {
    ...
    const { verifyResetOtp } = useAdminAuthStore();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (result.success) {
                toast({
                    title: "OTP VERIFIED! ✅",
                    description: "Identity confirmed. Proceed to reset password.",
                    variant: "success",
                });
        ...
        <div className="min-h-screen bg-background flex text-foreground relative overflow-hidden">
            ...
            <div className="w-full max-w-6xl mx-auto flex items-center justify-center p-4 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center w-full">
                    ...
                    >
                        <div className="mb-8">
                            <Link to="/admin/forgot-password" className="inline-flex items-center text-xs font-black uppercase tracking-widest text-muted-foreground hover:text-foreground mb-6 transition-colors group">
                                <ArrowLeft className="w-4 h-4 mr-2 group-hover:-translate-x-1 transition-transform" />
                                Back to Email
                            </Link>
                            <h1 className="text-4xl font-black italic uppercase tracking-tighter mb-2">Verify Identity</h1>
                            <p className="text-muted-foreground font-medium italic">Enter the 6-digit code sent to <b>{email || "your email"}</b>.</p>
                        </div>

                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-xs font-black uppercase tracking-widest pl-4">6-Digit OTP</label>
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        className="h-12 pl-12 rounded-2xl bg-secondary/10 border-secondary/20 focus:border-primary/50 text-sm font-bold tracking-[0.5em]"
                                        required
                                    />
                                </div>
                            </div>
                    ...
                    >
                        ...
                        <p className="text-xl text-muted-foreground font-medium max-w-md ml-auto">
                            Almost there. We just need to confirm you are the authorized partner for this account.
                        </p>
                    </motion.div>
                </div>
            </div>
