...
export default function ShippingSettings() {
  ...
  const saveSlab = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        showMessage(
          "success",
          editingSlabId ? "Slab updated!" : "Slab created!",
        );
        fetchData();
        closeSlabForm();
      }
      ...
      showMessage(
        "error",
        error.response?.data?.message || "Failed to save slab",
      );
    } finally {
      setSaving(false);
    }
  ...
  const deleteSlab = async (id) => {
    if (!window.confirm("Are you sure you want to delete this shipping slab?"))
      return;
  ...
  const toggleSlabStatus = async (slab) => {
    ...
    try {
      const res = await api.put(`/shipping/slabs/${slab._id}`, {
        status: newStatus,
      });
      if (res.data.success) {
        fetchData();
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-black italic tracking-tighter uppercase flex items-center gap-3">
            <Truck className="h-6 w-6 md:h-8 md:w-8 text-primary" /> Shipping
            Settings
          </h1>
          <p className="text-xs md:text-sm text-muted-foreground mt-1 font-medium italic">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="font-bold uppercase tracking-wide text-xs md:text-sm">
                      Free Shipping
                    </h3>
                  </div>
                  <p className="text-[10px] md:text-xs text-muted-foreground leading-relaxed">
                    Enable to offer FREE shipping on all orders. Shipping slabs
                    and COD charges will be ignored.
                  </p>
                </div>
                <button
                  onClick={() =>
                  ...
                  className={`relative w-14 h-7 md:w-16 md:h-8 rounded-full transition-all duration-300 ${
                    settings.freeShippingEnabled ? "bg-emerald-500" : "bg-muted"
                  }`}>
                  <div
                    className={`absolute top-1 w-5 h-5 md:w-6 md:h-6 bg-white rounded-full shadow-lg transition-all duration-300 ${
              ...
              {settings.freeShippingEnabled && (
                <div className="mt-4 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-2 text-emerald-600">
                  ...
                  <span className="text-[10px] md:text-xs font-bold uppercase tracking-wide">
                    Free shipping is active for all orders
                  </span>
                </div>
              )}
            </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`relative w-12 h-6 md:w-14 md:h-7 rounded-full transition-all duration-300 ${
                      settings.codEnabled ? "bg-primary" : "bg-muted"
                    }`}>
                    <div
                      className={`absolute top-0.5 w-5 h-5 md:w-6 md:h-6 bg-white rounded-full shadow-lg transition-all duration-300 ${
                        settings.codEnabled ? "left-6.5 md:left-7" : "left-0.5"
                      }`}></div>
                  </button>
                </div>
                ...
                {settings.codEnabled && (
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSettings({
                            ...settings,
                            codCharge: Number(e.target.value) || 0,
                          })
                        }
                    ...
                    <p className="text-[9px] md:text-[10px] text-muted-foreground">
                      This charge applies only when customer selects COD
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] md:text-xs text-muted-foreground">
                Define shipping charges based on order amount
              </p>
            </div>
          </div>
          ...
          <div className="mb-6 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl md:rounded-2xl flex items-start gap-3">
            ...
            <div>
              <p className="text-xs md:text-sm font-bold text-amber-600">
                Free Shipping is Active
              </p>
              <p className="text-[10px] md:text-xs text-amber-600/80">
                These slabs are currently bypassed because global free shipping
                is enabled.
              </p>
            </div>
          </div>
        ...
        {slabs.length === 0 ? (
          <div className="text-center py-12 md:py-16 bg-secondary/5 rounded-2xl border border-dashed border-secondary/30">
            ...
            <p className="text-muted-foreground font-medium text-sm md:text-base">
              No shipping slabs defined yet
            </p>
            <p className="text-[10px] md:text-xs text-muted-foreground mt-1">
              Add slabs to define shipping charges based on order amount
            </p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[600px]">
              <thead>
                <tr className="border-b border-border/50">
                  <th className="text-left py-3 md:py-4 px-3 md:px-4 text-[10px] md:text-xs font-black uppercase tracking-widest text-muted-foreground">
                    Order Range
                  </th>
                  <th className="text-center py-3 md:py-4 px-3 md:px-4 text-[10px] md:text-xs font-black uppercase tracking-widest text-muted-foreground">
                    Shipping Charge
                  </th>
                  <th className="text-center py-3 md:py-4 px-3 md:px-4 text-[10px] md:text-xs font-black uppercase tracking-widest text-muted-foreground">
                    Status
                  </th>
                  <th className="text-right py-3 md:py-4 px-3 md:px-4 text-[10px] md:text-xs font-black uppercase tracking-widest text-muted-foreground">
                    Actions
                  </th>
                </tr>
              </thead>
                  ...
                  <motion.tr
                    ...
                    <td className="py-3 md:py-4 px-3 md:px-4 text-center">
                      <button
                        ...
                        className={`inline-flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-widest transition-all ${
                          slab.status === "active"
                        ...
                        {slab.status === "active" ? (
                          <>
                            <ToggleRight className="h-3.5 w-3.5 md:h-4 md:w-4" />{" "}
                            Active
                          </>
                        ) : (
                          <>
                            <ToggleLeft className="h-3.5 w-3.5 md:h-4 md:w-4" />{" "}
                            Inactive
                          </>
                        )}
                      </button>
              ...
              <div className="space-y-4 md:space-y-6">
                <div className="grid grid-cols-2 gap-3 md:gap-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSlabForm({ ...slabForm, minAmount: e.target.value })
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSlabForm({ ...slabForm, maxAmount: e.target.value })
                      }
                      className="w-full h-10 md:h-12 px-3 md:px-4 bg-background border border-input rounded-xl outline-none focus:ring-2 focus:ring-primary/30 font-bold text-sm md:text-base"
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-3 md:gap-4">
                    <button
                      onClick={() =>
                        setSlabForm({ ...slabForm, status: "active" })
                      }
                      className={`flex-1 py-2.5 md:py-3 rounded-xl font-bold uppercase tracking-widest text-[10px] md:text-xs transition-all ${
                        slabForm.status === "active"
                          ? "bg-emerald-500 text-white"
                    ...
                    <button
                      onClick={() =>
                        setSlabForm({ ...slabForm, status: "inactive" })
                      }
                      className={`flex-1 py-2.5 md:py-3 rounded-xl font-bold uppercase tracking-widest text-[10px] md:text-xs transition-all ${
                        slabForm.status === "inactive"
                ...
                <button
                  onClick={saveSlab}
                  disabled={
                    saving || !slabForm.minAmount || !slabForm.shippingCharge
                  }
